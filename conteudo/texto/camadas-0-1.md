...
## Camadas 0 a 2 e capítulos 3.1–3.2 — texto integral
...
versões incompatíveis não devem ser misturados sem avaliação e estratégia de reindexação.
...
filtro e esta distribuição”. Um índice rápido que omite o documento decisivo falhou silenciosamente.
...
conjunto de perguntas reais, juízos de relevância e casos adversos — não com exemplos escolhidos
depois que o resultado já parece bom.

Num sistema de recuperação para IA, unidade de corte, metadados, versão da fonte e autorização
importam tanto quanto o índice. Conteúdo revogado precisa desaparecer das respostas; permissões
devem filtrar antes de expor; a citação precisa apontar para a evidência vigente. Embeddings podem
revelar relações sensíveis e têm custo de reprocessamento. O fundamento durável é tratar
recuperação como sistema de informação mensurável — não como memória infalível do modelo.
...
onde o tempo do evento avançou; políticas de atraso decidem quando emitir, corrigir ou descartar
resultados tardios.

O fluxo não elimina lote. Replay de um log limitado é um lote; materializar estado incremental é
uma forma de evitar recalcular tudo. A arquitetura precisa declarar retenção, posição do consumidor,
particionamento e ordem. Ordem global custa coordenação e raramente é necessária; ordem por chave
costuma expressar melhor o domínio. Reprocessar exige efeitos idempotentes ou um modo de separar
cálculo de publicação, porque corrigir o passado não pode duplicar o efeito no mundo.
...
*Refactoring Databases: Evolutionary Database Design*, 2006 · Malkov, Y. e Yashunin, D.,
[*Efficient and Robust Approximate Nearest Neighbor Search Using Hierarchical Navigable Small
World Graphs*](https://arxiv.org/abs/1603.09320), 2016 · Akidau, T. et al., [*The Dataflow
Model*](https://research.google/pubs/the-dataflow-model-a-practical-approach-to-balancing-correctness-latency-and-cost-in-massive-scale-unbounded-out-of-order-data-processing/),
VLDB, 2015 · Kleppmann, M., *Designing Data-Intensive Applications*, 2017.
...
Portabilidade passa a incluir a fronteira entre partes, não apenas a capacidade de executar o
mesmo módulo em máquinas diferentes.

Isso muda também a unidade de autoridade. Um componente pode declarar que precisa de relógio,
arquivos ou HTTP sem pressupor todo um sistema operacional. WASI fornece interfaces padronizadas,
e o runtime concede apenas capacidades escolhidas pelo hospedeiro. A segurança nasce da combinação
de isolamento de memória, validação, interface estreita e concessão explícita; executar em Wasm não
...
encerramento do suporte em 2020 · WebAssembly Community Group, [*Core
Specification*](https://webassembly.github.io/spec/core/) · Bytecode Alliance, [*The WebAssembly
Component Model*](https://component-model.bytecodealliance.org/) e [*WIT
Reference*](https://component-model.bytecodealliance.org/design/wit.html).
