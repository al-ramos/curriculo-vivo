...
<style>
:root{
  --ground:#FFFFFF; --surface:#FFFFFF; --ink:#1B201D; --muted:#5C645E; --faint:#88908A;
  ...
  --fonte-leitura:"Source Serif 4",Georgia,"Times New Roman",serif;
}
...
:root[data-tipo-fonte="sem-serifa"]{--fonte-leitura:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
:root[data-tipo-fonte="mono"]{--fonte-leitura:"IBM Plex Mono",ui-monospace,SFMono-Regular,Consolas,monospace}
...
.preferencias-status{margin:.85rem 0 0;font-family:"IBM Plex Mono",monospace;font-size:.62rem;
  letter-spacing:.04em;line-height:1.6;color:var(--muted)}
...
.nota textarea{width:100%;min-height:5.5rem;resize:vertical;background:var(--surface);
  ...
  font-family:var(--fonte-leitura);font-size:.92rem;line-height:1.5}
...
.strata .note{margin-top:.9rem;padding-top:.8rem;border-top:1px solid var(--rule-soft);
  font-family:var(--fonte-leitura);font-size:.85rem;color:var(--muted);line-height:1.5}
  ...
  <div class="mast-in">
    <div>
      ...
      <details class="preferencias"><summary>Preferências de leitura</summary><div class="ctrls">
        ...
        <div class="ctrl"><span>Tipo de letra</span>
          <div class="seg" role="group" aria-label="Tipo de letra para leitura">
            <button type="button" data-tipo-fonte="serifada">Serifada</button>
            <button type="button" data-tipo-fonte="sem-serifa">Sem serifa</button>
            <button type="button" data-tipo-fonte="mono">Mono</button>
          </div></div>
        <div class="ctrl"><span>Tema</span>
          <div class="seg" role="group" aria-label="Tema">
            <button type="button" data-tema="auto">Automático</button>
      ...
      </div><p class="preferencias-status" id="preferencias-status" aria-live="polite"></p></details>
      <div class="status">
...
(function(){
  ...
  function resumoPreferencias(){
    var el=document.getElementById('preferencias-status');
    if(!el) return;
    var caminho='cv-'+(location.pathname.split('/').pop() || 'index');
    var nomes={
      tema:{auto:'automático',light:'claro',dark:'escuro'},
      fonte:{menor:'menor',padrao:'padrão',maior:'maior',"extra-grande":'extra grande'},
      "tipo-fonte":{serifada:'serifada',"sem-serifa":'sem serifa',mono:'mono'},
      larg:{normal:'normal',largo:'larga'},
      secs:{abertas:'abertas',fechadas:'recolhidas'}
    };
    function nome(grupo,chave,padrao){ return nomes[grupo][ler(chave,padrao)] || nomes[grupo][padrao]; }
    el.textContent='Ativo: tema '+nome('tema','cv-tema','auto')+' · texto '+nome('fonte','cv-fonte','padrao')+
      ' · letra '+nome('tipo-fonte','cv-tipo-fonte','serifada')+' · coluna '+nome('larg','cv-larg','normal')+
      ' · seções '+nome('secs',caminho+'-secs','abertas')+'.';
  }
  ...
  function tema(v){
    ...
    marcar('tema',v); salvar('cv-tema',v); resumoPreferencias();
  }
  function fonte(v){ raiz.setAttribute('data-fonte',v); marcar('fonte',v); salvar('cv-fonte',v); resumoPreferencias(); }
  function tipoFonte(v){
    if(v==='serifada'){ raiz.removeAttribute('data-tipo-fonte'); } else { raiz.setAttribute('data-tipo-fonte',v); }
    marcar('tipo-fonte',v); salvar('cv-tipo-fonte',v); resumoPreferencias();
  }
  function largura(v){ corpo.classList.toggle('largo', v==='largo'); marcar('larg',v); salvar('cv-larg',v); resumoPreferencias(); }
  ...
  tipoFonte(ler('cv-tipo-fonte','serifada'));
  largura(ler('cv-larg','normal'));
  fichas(ler('cv-ficha','on'));
  recup(ler('cv-recup','on'));
  resumoPreferencias();
  document.addEventListener('cv:secoes', resumoPreferencias);

  document.addEventListener('click',function(e){
    var b=e.target.closest && e.target.closest('button[data-fonte],button[data-tipo-fonte],button[data-tema],button[data-larg],button[data-ficha],button[data-recup]');
    if(!b)return;
    if(b.hasAttribute('data-fonte')) fonte(b.getAttribute('data-fonte'));
    if(b.hasAttribute('data-tipo-fonte')) tipoFonte(b.getAttribute('data-tipo-fonte'));
...
(function(){
  ...
  function secoes(v){
    ...
    salvar('secs', v);
    document.dispatchEvent(new Event('cv:secoes'));
  }
