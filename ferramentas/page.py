...
.mast-in{display:grid;grid-template-columns:1fr;gap:2.5rem;padding:3.5rem 1.5rem 3rem;max-width:76rem;margin:0 auto}
...
.band{display:grid;grid-template-columns:5.5rem 1fr auto;align-items:center;gap:.75rem;padding:.3rem 0}
.band .lab{font-size:.72rem;color:var(--ink)}
.band .bar{height:100%;min-height:1.5rem;border-radius:1px;position:relative}
.band .hv{font-size:.66rem;color:var(--faint);white-space:nowrap}
.band .bar span{position:absolute;right:.45rem;top:50%;transform:translateY(-50%);
  font-size:.64rem;color:var(--ground);letter-spacing:.04em}
...
/* ---------- corpo ---------- */
.layout{display:grid;grid-template-columns:1fr;gap:3rem;padding:3.5rem 0 5rem}
...
article{min-width:0}
h2.camada{margin:4.5rem 0 .4rem;display:flex;flex-direction:column;gap:.15rem;
...
.cname{font-size:2rem;font-weight:300;letter-spacing:-.01em}
...
section.chapter h3 .ttl{font-size:1.55rem;font-weight:500;letter-spacing:-.01em;line-height:1.2}
...
.ficha{display:flex;flex-wrap:wrap;gap:0;margin:0 0 1.9rem;border:1px solid var(--rule);
  background:var(--surface);border-radius:2px;overflow:hidden}
.ficha>div{padding:.55rem .85rem;border-right:1px solid var(--rule-soft);flex:1 1 auto}
...
.rm-item{display:grid;grid-template-columns:2.6rem 1fr auto;gap:.8rem;align-items:baseline;
...
footer{border-top:1px solid var(--rule);color:var(--muted);font-size:.85rem;
  padding:2.5rem 1.5rem 4rem;max-width:76rem;margin:0 auto}
